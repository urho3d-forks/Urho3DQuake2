...
qboolean GL_Upload32 (unsigned *data, int width, int height,  qboolean mipmap)
{
    ...
    if (/*gl_round_down->value*/ false  && scaled_width > width && mipmap)
        scaled_width >>= 1;

    for (scaled_height = 1 ; scaled_height < height ; scaled_height<<=1)
        ;

    if (/*gl_round_down->value*/ false && scaled_height > height && mipmap)
        scaled_height >>= 1;
    ...
    */
    ...
    return false;//(samples == gl_alpha_format);
}
